...
def scrape_crime_data(location):
    url = "https://timesofindia.indiatimes.com/topic/{}-crime/{}?dateFilter=1577817000000,1609439399000"

    for page in range(1, 5):
        u = url.format(page,location)
        soup = BeautifulSoup(requests.get(u).content, "html.parser")
        crime_news = soup.find_all('div', class_='uwU81')
    ...
    # Check if any crime news was found
        ...
        for news in crime_news:
            title_element = news.find('div', class_='fHv_i o58kM')
            title = title_element.text.strip()
            date_text = news.find('div', class_='ZxBIG').text.strip()
            date_start = date_text.rfind('/') + 1
            date = date_text[date_start:].strip()
            if date == '':
              continue
            description_element = news.find('p', class_='oxXSK o58kM')
...
def save_crime_data_to_csv(crime_data, filename, location):
    ...
    month = datetime.strptime(crime_data[0]['date'], '%b %d, %Y, %H:%M (IST)').strftime('%m')
    ...
    with open(filename, 'a', newline='', encoding='utf-8') as csvfile:
        ...
        with open(f"{location}_uncategorized_crimes.txt", 'a', encoding='utf-8') as uncategorized_file:
            ...
            for i, data in enumerate(filtered_crime_data):
                ...
                if crime_type_count[crime_type] == 0:
                    ...
                    time = datetime.strptime(data['date'], '%b %d, %Y, %H:%M (IST)').strftime('%Y-%m-%d')
...
with open(filename, 'w', newline='', encoding='utf-8') as csvfile:
        csv_writer = csv.writer(csvfile)
        csv_writer.writerow(['location', 'area', 'time', 'crimetype', 'description', 'state', 'month'])
